...
<div class="flex justify-between gap-2 p-2">
    ...
    <div>
        <button type="button"
            class="hidden inline-flex items-center rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-medium leading-4 text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Export</button>
        <button wire:click="$set('openModal',true)" type="button"
            class="inline-flex items-center rounded-md border border-transparent bg-indigo-600 px-3 py-2 text-sm font-medium leading-4 text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Tambah
            Agenda</button>
            ...
            <x-slot name="content">
                <div class="space-y-5">
                    <div>
                        ...
                        <div class="mt-1">
                            <input wire:model="title" placeholder="Judul Agenda" type="text" name="agenda"
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <input wire:model="pic" placeholder="dipisahkan dengan , (Made, Ezra,...)" type="text"
                                name="agenda"
                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            @error('pic')
                                <span class="error">{{ $message }}</span>
                        ...
                        <fieldset class="mt-4" x-data="{ offline: false }">
                            <div class="space-y-4 sm:flex sm:items-center sm:space-y-0 sm:space-x-10">
                                <div class="flex items-center">
                                    <input wire:click="resetField()" wire:model="offline" value="0"
                                        @click="offline = false" name="notification-method" type="radio" checked
                                        class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                    <label class="ml-3 block text-sm font-medium text-gray-700">Online</label>
                                </div>

                                <div class="flex items-center">
                                    <input wire:model="offline" value="1" @click="offline = true"
                            ...
                            <div x-show="offline" class="pt-4">
                                <label for="agenda" class="block text-sm font-medium text-gray-700">Lokasi
                                    Offline</label>
                                <div class="mt-1">
                                    <input wire:model="lokasi_offline"
                                        placeholder="Masukan lantai, divisi, alamat, atau kosongkan saja" type="text"
                        ...
                        <fieldset>
                            <div x-data="{ checked: 'esokhari' }" class="-space-y-px rounded-md bg-white">
                                <label
                                    ...
                                    <input wire:model="is_tanggal" type="radio" name="privacy-setting" checked
                                        value="1"
                                    ...
                                    <span class="ml-3 flex flex-col">
                                        <span :class="checked === 'esokhari' ? 'text-indigo-900' : 'text-gray-900'"
                                            id="privacy-setting-0-label" class="block text-sm font-medium">Esok
                                            Hari</span>
                                        <span :class="checked === 'esokhari' ? 'text-indigo-700' : 'text-gray-500'"
                                            id="privacy-setting-0-description" class="block text-sm">Agenda ini akan
                                            dilakukan pada esok hari.
                                            {{ date('D, d M', mktime(0, 0, 0, date('m'), date('d') + 1, date('Y'))) }}</span>
                                    </span>
                                </label>
                                ...
                                <label class="relative border p-4 flex cursor-pointer focus:outline-none"
                                    ...
                                    <input wire:model="is_tanggal" type="radio" name="privacy-setting" value="0"
                                        class="mt-0.5 h-4 w-4 shrink-0 cursor-pointer text-indigo-600 border-gray-300 focus:ring-indigo-500"
                                        aria-labelledby="privacy-setting-1-label"
                                        aria-describedby="privacy-setting-1-description">
                                    <span class="ml-3 flex flex-col">
                                        <span :class="checked === 'hariini' ? 'text-indigo-900' : 'text-gray-900'"
                                            id="privacy-setting-1-label" class="block text-sm font-medium">Hari
                                            Ini</span>
                                        <span :class="checked === 'hariini' ? 'text-indigo-700' : 'text-gray-500'"
                                            id="privacy-setting-1-description" class="block text-sm">Agenda ini
                                            dilakukan pada hari ini. {{ date('D, d M') }}</span>
                                    </span>
                                </label>
                                ...
                                <label
                                    ...
                                    :class="checked === 'pilihtanggal' ? 'bg-indigo-50 border-indigo-200 z-10' :
                                        'border-gray-200'">

                                    <input wire:model="is_tanggal" type="radio" name="privacy-setting" value="2"
                                        class="mt-0.5 h-4 w-4 shrink-0 cursor-pointer text-indigo-600 border-gray-300 focus:ring-indigo-500"
                                    ...
                                    <span class="ml-3 flex flex-col">
                                        <span :class="checked === 'pilihtanggal' ? 'text-indigo-900' : 'text-gray-900'"
                                            ...
                                            class="block text-sm font-medium">Direncankan</span>
                                        <span :class="checked === 'pilihtanggal' ? 'text-indigo-700' : 'text-gray-500'"
                                            id="privacy-setting-2-description" class="block text-sm">Agenda ini akan
                                            dilakuakan sesuai dengan tanggal yang dipilih</span>
                                    </span>
                                </label>
                                <div x-show="checked ==='pilihtanggal' " class="pt-4">
                                    <label for="agenda" class="block text-sm font-medium text-gray-700">Pilih
                                        Tanggal</label>
                                    <div class="mt-1">
                                        <input wire:model="pil_tanggal" type="date" name="agenda"
                                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    </div>
                                </div>
                            </div>
    ...
    <div class="bg-white overflow-hidden sm:rounded-lg drop-shadow-md max-w-7xl mx-auto border-emerald-300">
        <div class="tabs">
            <div x-data="{
                ...
                <div
                    class="h-[32rem] w-full pt-4 overflow-auto scrollbar scrollbar-thumb-indigo-900 scrollbar-track-grey-100">
